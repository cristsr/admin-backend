...
- [x] **EP-1.1** Value objects: `AccountName` (jerarquía por nombre, 5 tipos
      raíz, validación de colisión), `Payee`, `PostingLine`, `Currency`.
- [x] **EP-1.2** Envelope de eventos de dominio (`event_id`, `aggregate_id`,
      `sequence`, `user_id`, `client_id`, `external_ref`, `occurred_at`,
      `recorded_at`) + serialización con montos como strings decimales (RNF-2).
- [x] **EP-1.3** Puerto `EventStore` (`append`/`load`/`readAll`) con su semántica:
      ...
      EP-0.4).
- [x] **EP-1.4** Adaptador **in-memory** de `EventStore` + contract tests.
- [x] **EP-1.5** Adaptador `PostgresEventStore` (esquema §6.1: tabla append-only,
      trigger de inmutabilidad, índices únicos, `projection_checkpoints`) que pasa
      **los mismos** contract tests (RNF-11).
- [x] **EP-1.6** Agregado `Account`: eventos `AccountOpened`/`Renamed`/`Closed`;
      invariantes INV-3 (parcial), INV-4, INV-13, INV-14.
- [x] **EP-1.7** Agregado `LedgerTransaction`: `TransactionRecorded`/`Amended`/
      `Annotated`/`Confirmed`/`Voided`/`Reversed`; INV-1 (balanceo cero por moneda
      en **componente único**, INV-11), INV-2, INV-6.
- [x] **EP-1.8** Command bus + políticas transversales (idempotencia, contexto,
...
- [x] **EP-1.9** Puerto `ReadModelStore` + `ProjectionDispatcher` (modo síncrono
      en transacción del command + poller con checkpoint; mismo código de proyector).
- [x] **EP-1.10** Proyectores núcleo: `account_tree`, `transaction_list`, `pending_review`
      (+ `proj_postings`), `account_balances`. Derivador de `derived_kind` (RF-4)
      como servicio de dominio.
- [x] **EP-1.11** Query bus + query handlers sobre esas proyecciones.
- [x] **EP-1.12** Tooling de **rebuild/replay** y verificación stream vs proyección
      (RNF-5).
...
- [x] **EP-2.1** Contrato OpenAPI inicial + versionado del API (RNF-8).
- [x] **EP-2.2** Adaptador HTTP driving: controllers → command bus / query bus.
- [x] **EP-2.3** Integración del **contexto autenticado** externo (`user_id`,
      `client_id`) vía guard/middleware; rechazo sin contexto válido (RF-26).
- [x] **EP-2.4** Endpoints de cuentas: `/ledger/initialize`, `/accounts`
      (crear/listar árbol/renombrar/cerrar/saldo).
- [x] **EP-2.5** Endpoints de transacciones: crear, listar/filtrar (incl. payee),
      `confirm`, `void`, `amend`, `annotate`, `reverse`.
- [x] **EP-2.6** Códigos de error de dominio estables (RF-14:
...
- [x] **EP-2.7** Lectura de escrituras propias (posición de stream / proyección
...
- [x] **EP-3.1** Agregado `BalanceAssertion`: `BalanceAsserted`/`Evaluated`/
      `Revoked`.
- [x] **EP-3.2** Servicio evaluador de aserciones con semántica temporal §2.4
...
- [x] **EP-3.3** Commands `AssertBalance`, `RevokeAssertion` + endpoints
      `/balance-assertions`, `/balance-assertions/{id}/revoke`.
- [x] **EP-3.4** Reactor de **re-evaluación** de aserciones (RF-18): process
      manager que escucha eventos y despacha commands, nunca escribe directo.
- [x] **EP-3.5** Resolución de discrepancias: command `ResolveDiscrepancy` →
      ajuste contra `Equity:Adjustments` + `DiscrepancyResolved`; endpoint
      `/balance-assertions/{id}/resolve`.
- [x] **EP-3.6** Proyecciones `assertion_status` y `adjustment_audit`.
...
- [x] **EP-4.1** `LedgerSettings`: `presentation_currency`, `timezone`
...
- [x] **EP-4.2** Catálogo de monedas: `CurrencyRegistered` + `/currencies`, con
...
- [x] ~~**Backups** automatizados del event store~~ — **resuelto a nivel de
      ...
      runbook.
- [ ] **Verificación de consistencia completa** (RNF-5): `ConsistencyVerifier` solo
      compara `proj_balances` contra el stream; el rebuild ya cubre las 8
      proyecciones. Auditoría del 2026-07-27, ver `decisions.md`.
