...
export function PickInputType<T, K extends keyof T>(
  classRef: Type<T>,
  keys: readonly K[] = []
): Type<Pick<T, typeof keys[number]>> {
  return PickType<T, K>(classRef, keys, InputType);
}

export function PickObjectType<T, K extends keyof T>(
  classRef: Type<T>,
  keys: readonly K[]
): Type<Pick<T, typeof keys[number]>> {
  return PickType(classRef, keys, ObjectType);
}
