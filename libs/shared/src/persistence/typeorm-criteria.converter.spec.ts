...
const toWhere = (criteria: Criteria<Field>) =>
  TypeOrmCriteriaConverter.toWhere<Record<string, unknown>, Field>(fields, criteria);

/** The operator type TypeORM will render, e.g. `moreThan`. */
const typeOf = (value: unknown) => (value instanceof FindOperator ? value.type : value);

describe('TypeOrmCriteriaConverter', () => {
  it('maps each domain field to the property path that backs it', () => {
    const where = toWhere(empty().equals('account', 3)) as any;
  ...
  it('matches the whole value for a case-insensitive equality', () => {
    const where = toWhere(empty().equalsIgnoreCase('merchant', 'Uber')) as any;
  ...
  it('ANDs two conditions on the same column into one range', () => {
    const where = toWhere(empty().greaterThan('amount', 10).lessThan('amount', 100)) as any;
  ...
  it('turns a null check into IS NULL rather than a comparison with null', () => {
    const where = toWhere(empty().isNull('merchant')) as any;
    const notNull = toWhere(empty().isNotNull('merchant')) as any;
  ...
  it('carries ordering and paging into the find options', () => {
    const options = TypeOrmCriteriaConverter.toFindOptions<Record<string, unknown>, Field>(
      fields,
      empty()
  ...
  it('refuses a field the adapter never mapped', () => {
    const unmapped = { id: 'id' } as CriteriaFieldMap<Field>;

    expect(() =>
      TypeOrmCriteriaConverter.toWhere<Record<string, unknown>, Field>(unmapped, empty().equals('amount', 1)),
    ).toThrow(InvalidCriteriaException);
  });

  it('renders a membership filter as IN', () => {
    const where = toWhere(empty().where('id', FilterOperator.IN, [1, 2, 3])) as any;
