...
import { QueryPorts } from './read-side-ports.factory';
 ...
 *
 * Every handler reads through a typed port, so the bus no longer sees the
 * `ReadModelStore` at all. The caller states the ports — the store-backed
 * composition from `createQueryPorts`, or a composition that substitutes an
 * adapter the shared store cannot serve (the SQL `TransactionFinder`, R7).
 *
...
export function createQueryBus(ports: QueryPorts): RegistryQueryBus {
  ...
  bus.register(ListTransactionsQuery, new ListTransactionsHandler(ports.transactions));
  bus.register(GetTransactionByIdQuery, new GetTransactionByIdHandler(ports.transactions));
  bus.register(ListPendingReviewQuery, new ListPendingReviewHandler(ports.pendingReview));
  bus.register(ListCurrenciesQuery, new ListCurrenciesHandler(ports.currencies));
