...
describe('MergePendingTransfersHandler', () => {
  ...
  it("preserves both legs' external references in metadata", async () => {
    const { handler, transactions, bus } = setup();
    seedMergeablePair(transactions);
  ...
  it('keeps the positional slot of a leg with no external reference', async () => {
    const { handler, transactions, bus } = setup();
    seedMergeablePair(transactions);
    transactions.externalRefOf.mockImplementation(async (_userId, id) =>
  ...
  it('omits the external references when neither leg carries one', async () => {
    const { handler, transactions, bus } = setup();
    seedMergeablePair(transactions);
  ...
  it('emits TransfersMerged on the resulting transfer', async () => {
