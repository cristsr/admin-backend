 ...
 * transfer (§3.4), inside the same scope.
 *
...
export class MergePendingTransfersHandler extends CommandHandler<MergePendingTransfersCommand> {
  ...
  /**
   * Traceability of the merge (RF-16): the ids of the two voided pendings plus
  ...
  /**
   * Emits {@link TransfersMerged} on the transfer just recorded (§3.4). It is
   ...
   * a read-your-writes caller has to wait for (RNF-9).
   */
   ...
   * never reads a projection (§6.3, RNF-10).
   */
