 ...
 * → TRANSFER; anything else → COMPOUND. Never throws — an unclassifiable
 * mix is COMPOUND, not an error.
 *
 * Two undefined cases resolve to COMPOUND rather than to an
 * arbitrary concrete kind, following the protection rule ("unknown
 ...
 * - **EXPENSES and INCOME together.** There is no precedence between them;
