...
/**
 * Serves {@link AdjustmentAuditReader} from `proj_adjustment_audit` through the
 * shared {@link ReadModelStore}.
 */
@Injectable()
export class ReadModelAdjustmentAuditReader extends AdjustmentAuditReader {
  constructor(private readonly store: ReadModelStore) {
    super();
  }

  async byAccount(userId: string, accountId: string): Promise<readonly AdjustmentAuditRecord[]> {
