...
import { MovementSource, MovementType } from './movement.types';
...
describe('Movement', () => {
  describe('named constructors', () => {
    ...
    it('stamps the source that recorded it', () => {
      ...
      expect(Movement.fromSchedule(attributes).source).toBe(
        MovementSource.SCHEDULED,
      );
      expect(
  ...
  describe('applyPatch (AC-4)', () => {
    ...
    it('refuses to edit a transfer leg', () => {
      ...
      expect(() => leg.applyPatch({ notes: 'x' })).toThrow(
    ...
    it('refuses to edit ingestion-owned fields on a webhook movement', () => {
      ...
      expect(() => ingested.applyPatch({ amount: 999 })).toThrow(
        MovementNotEditableException,
      );
      expect(() => ingested.applyPatch({ description: 'x' })).toThrow(
        MovementNotEditableException,
      );
    });
    ...
    it('ignores keys explicitly set to undefined', () => {
      ...
      expect(() =>
        ingested.applyPatch({ amount: undefined, notes: 'mine' }),
      ).not.toThrow();
    });
  });
