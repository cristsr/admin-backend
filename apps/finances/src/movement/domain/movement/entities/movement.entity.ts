...
import { IngestedMovement } from '../types/ingested-movement.type';
import { MovementPatch } from '../types/movement-patch.type';
import {
...
import { NewMovement } from '../types/new-movement.type';
import { TransferLeg } from '../types/transfer-leg.type';
...
const TRANSFER_TYPES = new Set<MovementType>([MovementType.TRANSFER_IN, MovementType.TRANSFER_OUT]);

/** Types that take money out of the account holding them. */
const WITHDRAWAL_TYPES = new Set<MovementType>([MovementType.EXPENSE, MovementType.TRANSFER_OUT]);
...
export class Movement {
  ...
  private oppositeFlowType(): MovementType {
    return this.type === MovementType.EXPENSE ? MovementType.INCOME : MovementType.EXPENSE;
  }

  private oppositeTransferType(): MovementType {
    return this.type === MovementType.TRANSFER_OUT ? MovementType.TRANSFER_IN : MovementType.TRANSFER_OUT;
  }
  ...
  private ensureEditable(patch: MovementPatch): void {
    ...
    const forbidden = Object.keys(patch).filter(
      (field) => patch[field] !== undefined && !INGESTION_EDITABLE.has(field as keyof MovementPatch),
    );
