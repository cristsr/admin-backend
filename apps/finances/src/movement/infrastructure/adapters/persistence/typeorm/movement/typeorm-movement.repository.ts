...
import { Movement, MovementField, MovementRepository } from '@app/movement/domain/movement';
...
export class TypeOrmMovementRepository implements MovementRepository {
  ...
  async firstMatching(criteria: Criteria<MovementField>): Promise<Nullable<Movement>> {
  ...
  async countMatching(criteria: Criteria<MovementField>): Promise<number> {
    return this.repository.countBy(
      TypeOrmCriteriaConverter.toWhere<TypeOrmMovementEntity, MovementField>(
        MOVEMENT_CRITERIA_FIELDS,
        criteria,
      ),
    );
  ...
  async save(movement: Movement): Promise<Movement> {
    const saved = await this.repository.save(TypeOrmMovementMapper.toEntity(movement));
    const entity = await this.repository.findOne({
      where: { id: saved.id },
      relations: RELATIONS,
    });
  ...
  async saveWithManager(manager: EntityManager, movement: Movement): Promise<Movement> {
    const saved = await manager.save(TypeOrmMovementEntity, TypeOrmMovementMapper.toEntity(movement));
  ...
  async saveAll(movements: Movement[]): Promise<Movement[]> {
    return this.repository.manager.transaction(async (manager) => {
      const saved = await manager.save(TypeOrmMovementEntity, movements.map(TypeOrmMovementMapper.toEntity));
      return saved.map((entity) => TypeOrmMovementMapper.toDomain(entity as TypeOrmMovementEntity));
    });
  }
