...
describe('Account', () => {
  ...
  describe('ensureCanWithdraw', () => {
    it('refuses a withdrawal the balance cannot cover', () => {
      expect(() => buildAccount().ensureCanWithdraw(Money.of(150, 'COP'), 0)).toThrow(
        InsufficientBalanceException,
      );
    });

    it('allows a withdrawal that exactly empties the account', () => {
      expect(() => buildAccount().ensureCanWithdraw(Money.of(100, 'COP'), 0)).not.toThrow();
    });
    ...
    it('allows any withdrawal on an account that may go negative', () => {
      ...
      expect(() => creditCard.ensureCanWithdraw(Money.of(9999, 'COP'), 0)).not.toThrow();
    });

    it('counts the movements, not just the opening balance', () => {
      expect(() => buildAccount().ensureCanWithdraw(Money.of(50, 'COP'), -80)).toThrow(
        InsufficientBalanceException,
      );
    });
  });
