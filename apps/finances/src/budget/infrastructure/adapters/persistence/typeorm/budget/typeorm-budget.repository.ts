...
import { Budget, BudgetField, BudgetRepository } from '@app/budget/domain/budget';
...
export class TypeOrmBudgetRepository implements BudgetRepository {
  ...
  async matching(criteria: Criteria<BudgetField>): Promise<Budget[]> {
    const entities = await this.repository.find({
      ...TypeOrmCriteriaConverter.toFindOptions<TypeOrmBudgetEntity, BudgetField>(
  ...
  async firstMatching(criteria: Criteria<BudgetField>): Promise<Nullable<Budget>> {
  ...
  async save(budget: Budget): Promise<Budget> {
    const saved = await this.repository.save(TypeOrmBudgetMapper.toEntity(budget));
  ...
  async removeMatching(criteria: Criteria<BudgetField>): Promise<number> {
    const result = await this.repository.softDelete(
      TypeOrmCriteriaConverter.toWhere<TypeOrmBudgetEntity, BudgetField>(BUDGET_CRITERIA_FIELDS, criteria),
    );
